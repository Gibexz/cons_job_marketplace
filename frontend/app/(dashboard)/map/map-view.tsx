...
export default function MapView() {
  ...
  useEffect(() => {
    ...
    // ✅ Wait for style to fully load before adding markers
    map.on("load", () => {
      loadJobs(map);
      loadWorkers(map);
    });
  ...
  async function loadJobs(map: mapboxgl.Map) {
    try {
      ...
      jobs.forEach((job: any) => {
        ...
        el.className =
          "flex h-8 w-8 cursor-pointer items-center justify-center rounded-full bg-blue-600 text-white shadow-md text-sm border-2 border-white";
  ...
  async function loadWorkers(map: mapboxgl.Map) {
    try {
      const workers = await apiFetch("/worker-profile/map");
      workers.forEach((worker: any) => {
        // Styled worker marker
        const el = document.createElement("div");
        el.className =
          "flex h-8 w-8 items-center justify-center rounded-full bg-[#ff6600] text-white shadow-md text-sm border-2 border-white";
